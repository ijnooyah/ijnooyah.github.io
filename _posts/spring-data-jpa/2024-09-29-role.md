...
애플리케이션 시작 시 기본적인 사용자 권한명을 추가하는 역할을 하는 클래스다.
<br>
...
- Spring Boot의 `CommandLineRunner` 인터페이스를 구현한다.
- 이 인터페이스는 애플리케이션이 시작될 때 실행되어야 하는 코드를 정의할 수 있게 해준다.

**initializeRoles()**
- `roleRepository.count() == 0`로 데이터베이스에 권한명이 없는 경우에만 초기화를 수행한다.
